...
import {
  Settings2, Link2, Key, Download, Copy, RefreshCw,
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <Label className="flex items-center gap-1.5 text-sm">
            <Link2 className="h-3.5 w-3.5" /> WordPress Site URL
          </Label>
          <Input placeholder="https://your-site.com" value={wpUrl} onChange={(e) => setWpUrl(e.target.value)} />
        ...
        <div className="space-y-2">
          ...
          {apiKey ? (
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="icon" onClick={handleCopyKey}><Copy className="h-4 w-4" /></Button>
              <Button variant="outline" size="icon" onClick={handleGenerateKey}><RefreshCw className="h-4 w-4" /></Button>
            </div>
          ) : (
            <Button variant="outline" onClick={handleGenerateKey} disabled={isGenerating}>
              {isGenerating ? <><RefreshCw className="h-4 w-4 mr-1.5 animate-spin" /> Generating...</> : <><Key className="h-4 w-4 mr-1.5" /> Generate API Key</>}
            </Button>
          )}
        </div>
        ...
        <div className="space-y-3">
          <Label className="flex items-center gap-1.5 text-sm">
            <Download className="h-3.5 w-3.5" /> WordPress Plugin
          </Label>
          <p className="text-xs text-muted-foreground">
            Download and install the ImageVault plugin on your WordPress site.
          </p>
          ...
          <div className="rounded-md border bg-muted/50 p-4 space-y-2">
            <p className="text-sm font-medium flex items-center gap-1.5">
              <AlertCircle className="h-3.5 w-3.5" /> Installation instructions
            </p>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleConnect} disabled={!wpUrl.trim() || !apiKey || isConnected}>
            {isConnected ? <><CheckCircle2 className="h-4 w-4 mr-1.5" /> Connected</> : <><Link2 className="h-4 w-4 mr-1.5" /> Connect</>}
          </Button>
        </div>
      </CardContent>
    </Card>
